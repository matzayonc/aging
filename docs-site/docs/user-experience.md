...
Everything so far (Mental Model #1, Position Market, Invariants) describes
the mechanism — positions, tranches, the primary market and the per-band
order book. This page is about what different kinds of users actually see
and do on top of it.
...
directly with the **position market**: the per-band order books described in
[Position Market](./position-market.md). They place orders on specific
bands, on specific positions, the same way any order-book participant
would. This is also who's expected to do the market-making that keeps a
thin band's price reasonable — with no primary-market peg to anchor it,
that's harder than ordinary market-making, and it's an open question in
[Position Market](./position-market.md#open-questions) how well it holds
up.
